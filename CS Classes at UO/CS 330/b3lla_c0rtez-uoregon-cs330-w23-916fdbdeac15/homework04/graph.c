...
void construct_adj_list(int** adj_mat, int rows, int cols, adj_node_t*** list)
{
    ...
    for (int i = 0; i < rows; i++) {
	adj_node_t* node_i = create_node(i);
    	add_node(myList, i, node_i);
	for (int j = 0; j<rows; j++) {
		if(adj_mat[i][j] != 0) {
			adj_node_t* node_j = create_node(j);
			add_node(myList, i, node_j);
		}
	}
    }
         ...
         int* color, int* distance, int* parent)
{
    ...
    adj_node_t* new_source = create_node(source);
    
    add_node(queue, 0, new_source); 

    while (*queue != NULL) { 
    	int u = remove_node(queue);

	for (adj_node_t* next = list[u]; next != NULL; next = next->next) {
		int v = next->vid;
