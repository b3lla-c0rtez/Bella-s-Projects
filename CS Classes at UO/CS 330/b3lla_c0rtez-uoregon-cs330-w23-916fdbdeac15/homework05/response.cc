...
bool Response::checkAndRespond(const string& inWord, ostream& toWhere)
{
	...
	size_t found_message = inWord.find(keyword.normal());
	if (found_message != string::npos) {
		kw_check = true;
		respond(toWhere);
	}
	return kw_check;
...
{
	string rp = "I am neither angry nor happy: " + this->response + "\n";
	toWhere << rp;
...
{
	string rp = "I am angry: " + this->response + "\n";
	toWhere << rp;
...
void HappyResponse::respond(ostream& toWhere) {
	string rp = "I am happy: " + this->response + "\n";
	toWhere << rp;
