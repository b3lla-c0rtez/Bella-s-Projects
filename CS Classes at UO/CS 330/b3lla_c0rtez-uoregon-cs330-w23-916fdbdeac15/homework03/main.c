...

{
  ...
  *csr_vals = (double *)malloc(sizeof(double)*(nnz));
  ...
  unsigned int hist[m];
  memset(hist, 0, sizeof(unsigned int) * m);
  ...
  for (int j = 0; j <= m; j++) {
	  if (j == 0) {
		...
		(*csr_row_ptr)[j] = hist[j-1] + (*csr_row_ptr)[j-1];
	  }
  ...
  memset(hist,0,sizeof(unsigned int) * m);
  for (int k = 0; k<nnz; k++) {
	  int csr_ind = hist[row_ind[k]-1] + (*csr_row_ptr)[row_ind[k]-1];
	  (*csr_col_ind)[csr_ind] = col_ind[k];
	  (*csr_vals)[csr_ind] = val[k];
	  hist[row_ind[k]-1] += 1;
  }
}
