...
Credits: Lauren Matthews, Catherine Raj, Quinn Rainen, Micah Nichols, Levin Neumann

Description: A solver for the classic puzzle, as a programming project for CIS 211 at University of Oregon.
The project will have two phases.
In the first phase we implement constraint propagation to solve simple puzzles.
In the second phase we will add back-tracking search to solve harder puzzles.
...
class Board(object):
    ...
    def solve(self):
        ...
        if self.is_complete():
            ...
            board = self.as_list()
            r_tile = self.min_choice_tile()
            for value in r_tile.candidates:
                r_tile.set_value(value)
                if self.solve():
                    return True
                else:
    ...
    def naked_single(self) -> bool:
        ...
        for group in self.groups:
            popped_value = set()
            for tile in group:
                popped_value.add(tile.value)
            for tile in group:
                if tile.remove_candidates(popped_value):
                    value = True
        return value
    ...
    def hidden_single(self):
        for group in self.groups:
            ...
            for value in leftovers:
                ...
                if count == 1:
                    for tile in group:
                        if tile.value is UNKNOWN and value in tile.candidates:
    ...
    def min_choice_tile(self) -> Tile:
        ...
        for group in self.groups:
            for tile in group:
                if tile.value == UNKNOWN and len(tile.candidates) == 2:
                    return tile
